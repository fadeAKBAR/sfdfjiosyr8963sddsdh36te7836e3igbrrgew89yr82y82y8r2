  ...
  return (
    <section className="bg-white border border-gray-100 rounded-2xl p-6 shadow-xs mb-6 text-[#1A1A1A]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {selectedMethod === 'balance' && !isBalanceSufficient && selectedPackage && (
            <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between text-xs text-amber-900">
              <div className="flex items-center gap-2">
                ...
                <span>
                  Your balance (${usdBalance.toFixed(2)}) is short by ${(price - usdBalance).toFixed(2)}
                </span>
              </div>
              <button
                type="button"
        ...
        <div className="lg:col-span-5 bg-black rounded-2xl p-6 text-white shadow-md flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-5 pb-3 border-b border-neutral-800">
              <span className="text-xs font-semibold tracking-wider text-gray-400 uppercase">
                Order Summary
              </span>
              <span className="text-[10px] bg-white/20 px-2 py-1 rounded text-white font-bold tracking-tighter">
                SECURE SSL
              </span>
            </div>
            ...
            <div className="space-y-3.5 text-sm">
              ...
              <div className="flex items-center justify-between text-gray-300">
                <span className="text-xs text-gray-400">Coins Package:</span>
              ...
              <div className="flex items-center justify-between text-gray-300">
                <span className="text-xs text-gray-400">Package Rate:</span>
              ...
              <div className="pt-4 border-t border-neutral-800 flex items-baseline justify-between">
                <div>
                  <span className="block font-bold text-white text-lg">Total Cost:</span>
                  <span className="text-[10px] text-gray-400">Includes all taxes & gateway fees</span>
                </div>
                <div className="text-right">
                  <span className="font-black text-3xl text-[#FE2C55]">
                    ${price.toFixed(2)}
                  </span>
                  <span className="text-[10px] text-gray-400 block font-semibold">USD</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-neutral-800">
            <button
              ...
              className="w-full py-4 bg-white text-black font-extrabold text-base rounded-xl hover:bg-gray-100 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed transition flex items-center justify-center gap-2 shadow-sm"
            >
              {isProcessing ? (
                <>
                ...
                <>
                  ...
                  <span>Complete Purchase (${price.toFixed(2)} USD)</span>
