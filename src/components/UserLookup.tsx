    ...
    <section className="bg-white border border-gray-100 rounded-2xl p-6 shadow-xs mb-6 text-[#1A1A1A]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
        ...
        {user && (
          <button
            ...
            onClick={handleClear}
            className="self-start sm:self-auto text-xs text-gray-500 hover:text-black flex items-center gap-1.5 py-1.5 px-3 rounded-xl bg-gray-100 hover:bg-gray-200 transition font-bold"
          >
      ...
      >
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-8 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl text-[#1A1A1A] placeholder-gray-400 font-medium focus:outline-none focus:border-[#FE2C55] focus:bg-white transition text-sm sm:text-base"
          />
        </div>
        ...
        <button
          ...
          className="px-6 py-3.5 rounded-xl bg-black hover:bg-neutral-800 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed font-bold text-white text-sm flex items-center justify-center gap-2 transition shadow-xs"
        >
          {isSearching ? (
            <>
      ...
      {!user && (
        <div className="mt-3 flex items-center flex-wrap gap-1.5 text-xs">
          <span className="text-gray-400 mr-1 font-medium">Examples:</span>
          {quickSuggestions.map((item) => (
            <button
              ...
              className="px-3 py-1 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 hover:text-black transition font-medium"
            >
              <span>@{item}</span>
            </button>
          ))}
      ...
      {user && (
        <div
          id="tiktok-user-card"
          className="mt-5 p-4 bg-[#F8FAFC] rounded-2xl border border-gray-200/90 flex items-center justify-between gap-4 animate-in fade-in duration-200"
        >
          <div className="flex items-center space-x-3.5">
            <div className="relative shrink-0">
              <img
                ...
                className="w-13 h-13 rounded-full bg-white border-2 border-white object-cover shadow-sm ring-2 ring-gray-100"
              />
              <span
            ...
            <div className="flex items-center gap-2">
              <span className="font-semibold text-base sm:text-lg text-[#1A1A1A]">
                {user.username}
              </span>
              {user.isVerified && (
                <div className="flex items-center gap-1 px-2 py-0.5 bg-[#20D5EC]/15 rounded-full text-[11px] font-bold text-cyan-800">
