...
export const HistoryModal: React.FC<HistoryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-2xl text-[#1A1A1A] max-h-[85vh] flex flex-col">
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-extrabold text-base text-[#1A1A1A]">Coin Transaction History</h3>
          ...
          <div className="flex items-center gap-2">
            {transactions.length > 0 && (
              <button
                ...
                className="text-xs text-rose-600 hover:text-rose-700 p-1.5 rounded-lg hover:bg-rose-50 flex items-center gap-1 transition font-bold cursor-pointer"
                title="Clear history"
              >
        ...
        <div className="p-4 overflow-y-auto flex-1 space-y-3">
          {transactions.length === 0 ? (
            ...
            transactions.map((tx) => (
              <div
                key={tx.id}
                className="p-4 rounded-xl bg-gray-50 border border-gray-200/80 hover:border-gray-300 transition"
              >
